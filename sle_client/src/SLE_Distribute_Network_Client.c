...
/* Observable state for LCD (mirrors server's g_sle_state convention):
 *   0 init, 1 enabled+scanning, 2 server found+connecting, 3 connected,
 *   4 creds received, 5 wifi connecting, 7 ALL OK, 10 enable_sle fail, 14 wifi fail */
volatile int g_sle_client_state = 0;

const char *sle_client_get_ssid(void) { return g_ssid; }
...
static void example_wifi_notification_cbk(ssapc_handle_value_t *data)
{
    ...
    if (data->data_len > NTF_IND_FLAG_MAX_LEN &&
        ...
        g_sle_client_state = 4;
        PRINT("[SLE Client] credentials received: ssid=%s\r\n", g_ssid);
...
static int example_sle_distribute_network_client_task(const char *arg)
{
    ...
    if (enable_sle() != ERRCODE_SUCC) {
        ...
        g_sle_client_state = 10;
        return -1;
    }

    g_sle_client_state = 1;
    return 0;
}
...
/* Exposed to app_demo.c. The original `app_run(...)` registration was
 * removed so the orchestrator picks which role to start. */
...
#if 0  /* original app_run was here — kept commented for reference */
